...
// Exisiting flat-field configs auto-migrate to the connections array on load.
//
...
async function getConfig() {
  ...
  if (!raw.connections) {
    ...
    if (flat.serverURL) {
      ...
      await chrome.storage.local.set({ connections: raw.connections, activeConnectionId: raw.activeConnectionId });
    } else {
      raw.connections = [];
    }
  }
...
async function updateBadge() {
  ...
  if (!tab || !tab.url) { await chrome.action.setBadgeText({ text: '' }); return; }
  if (!store.isUnlocked()) { await chrome.action.setBadgeText({ text: '' }); return; }
  try {
...
async function handleMessage(message, sender) {
  switch (message.type) {
    ...
    case 'REMOVE_CONNECTION': {
      ...
      await chrome.storage.local.set({ connections: cfg.connections, activeConnectionId: cfg.activeConnectionId });
      return { ok: true };
    }
...
chrome.runtime.onInstalled.addListener(() => {
  chrome.contextMenus.create({ id: 'autofill', title: 'Fill credentials from PlusEV Secrets', contexts: ['editable'] });
});
